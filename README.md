...
TODO Instructions to get up&running
...
An `AnagramInput` acts as a wrapper for inputs that are valid according to the aforementioned definition (i.e. no
`AnagramInput` wrapping invalid inputs can be created).
...
that could be cleared before exiting the program to start fresh every time.
