...
public class AnagramCommand {
  ...
  @Command(command = "history", description = "Given an input text, returns its known anagrams among all the texts "
                                              + "provided to the check command during this session")
  public String history(String input) {
    Text ai = new Text(input);
    return "List of known anagrams %s".formatted(
      anagramService.historicalCheck(ai).stream().map(Text::input).collect(Collectors.toSet()));
  }
